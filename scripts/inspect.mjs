...
(async () => {
  ...
  if (!path) {
    ...
    console.error(
      'Environment: set DEV_BASE_URL to override dev base (default http://localhost:8866)'
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (format === 'json') {
      ...
      if (result.summary.touchspinInitialized + result.summary.touchspinNotInitialized > 0) {
        console.log(
          `TouchSpin Initialized: ${result.summary.touchspinInitialized}/${result.summary.touchspinInitialized + result.summary.touchspinNotInitialized}`
        );
      }
    }
  } catch (error) {
  ...
  const hasErrors =
    result.summary.totalPageErrors > 0 ||
