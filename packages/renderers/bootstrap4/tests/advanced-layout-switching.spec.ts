...
const BOOTSTRAP3_FIXTURE = '/packages/renderers/bootstrap4/tests/fixtures/bootstrap4-fixture.html';
...
test('rebuilds DOM correctly when switching layouts in advanced mode', async ({ page }) => {
  ...
  // Verify initial horizontal order: prepend-wrapper (down+prefix) -> input -> append-wrapper (postfix+up)
  const initialOrder = await getElementOrder();
  expect(initialOrder).toEqual(['prepend-wrapper', 'input', 'append-wrapper']);
  ...
  // Verify element order is restored: prepend-wrapper -> input -> append-wrapper
  const finalOrder = await getElementOrder();
  expect(finalOrder).toEqual(['prepend-wrapper', 'input', 'append-wrapper']);
