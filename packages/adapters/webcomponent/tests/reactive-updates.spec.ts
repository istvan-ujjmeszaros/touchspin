...
test.describe('TouchSpin Web Component reactive attribute updates', () => {
  ...
  test('processes nested attribute structures', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      element.setAttribute(
        'structured-data',
        JSON.stringify({ validation: { required: true, min: 0 } })
      );
      element.setAttribute(
        'nested-settings',
        JSON.stringify({ ui: { buttons: { style: 'rounded' } } })
      );
      document.body.appendChild(element);
    });
    ...
    await page.evaluate(() => {
      ...
      if (element) {
        element.setAttribute(
          'complex-config',
          JSON.stringify({ theme: 'light', size: 'sm', variant: 'outlined' })
        );
        element.setAttribute(
          'structured-data',
          JSON.stringify({ validation: { required: false, max: 100 } })
        );
        element.setAttribute(
          'nested-settings',
          JSON.stringify({ ui: { buttons: { style: 'square', size: 'compact' } } })
        );
      }
    });
    ...
    expect(JSON.parse(nestedTest.complexConfig || '{}')).toEqual({
      theme: 'light',
      size: 'sm',
      variant: 'outlined',
    });
    expect(JSON.parse(nestedTest.structuredData || '{}')).toEqual({
      validation: { required: false, max: 100 },
    });
    expect(JSON.parse(nestedTest.nestedSettings || '{}')).toEqual({
      ui: { buttons: { style: 'square', size: 'compact' } },
    });
  ...
  test('updates event configuration attributes', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      element.setAttribute(
        'callback-config',
        JSON.stringify({ onChange: 'logValue', onInput: 'validate' })
      );
    ...
    await page.evaluate(() => {
      ...
      if (element) {
        ...
        element.setAttribute(
          'callback-config',
          JSON.stringify({ onChange: 'format', onBlur: 'save' })
        );
    ...
    expect(JSON.parse(eventTest.updatedCallbacks || '{}')).toEqual({
      onChange: 'format',
      onBlur: 'save',
    });
  ...
  test('handles internationalization attribute changes', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      element.setAttribute(
        'number-format',
        JSON.stringify({ style: 'decimal', minimumFractionDigits: 2 })
      );
    ...
    await page.evaluate(() => {
      ...
      if (element) {
        ...
        element.setAttribute(
          'number-format',
          JSON.stringify({ style: 'currency', currency: 'EUR' })
        );
    ...
    expect(JSON.parse(i18nTest.updatedNumberFormat || '{}')).toEqual({
      style: 'currency',
      currency: 'EUR',
    });
  ...
  test('processes plugin-specific attribute updates', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      element.setAttribute(
        'extension-settings',
        JSON.stringify({ autoSave: true, validateOnBlur: false })
      );
    ...
    await page.evaluate(() => {
      ...
      if (element) {
        element.setAttribute(
          'plugin-config',
          JSON.stringify({ name: 'formatter', version: '2.0', enabled: true })
        );
        element.setAttribute(
          'extension-settings',
          JSON.stringify({ autoSave: false, validateOnBlur: true, format: 'currency' })
        );
    ...
    expect(JSON.parse(pluginTest.updatedConfig || '{}')).toEqual({
      name: 'formatter',
      version: '2.0',
      enabled: true,
    });
    expect(JSON.parse(pluginTest.updatedSettings || '{}')).toEqual({
      autoSave: false,
      validateOnBlur: true,
      format: 'currency',
    });
  ...
  test('maintains backward compatibility during updates', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      element.setAttribute(
        'deprecated-options',
        JSON.stringify({ oldSetting: true, legacyMode: 'v1' })
      );
    ...
    await page.evaluate(() => {
      ...
      if (element) {
        element.setAttribute(
          'deprecated-options',
          JSON.stringify({ oldSetting: false, legacyMode: 'v2', newFeature: true })
        );
    ...
    expect(JSON.parse(legacyTest.updatedOptions || '{}')).toEqual({
      oldSetting: false,
      legacyMode: 'v2',
      newFeature: true,
    });
  ...
  test('updates framework-specific attributes', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      element.setAttribute(
        ...
        JSON.stringify({ reactivity: 'auto', binding: 'two-way' })
      );
    ...
    await page.evaluate(() => {
      ...
      if (element) {
        element.setAttribute(
          'framework-config',
          JSON.stringify({ type: 'vue', version: '3.3', features: ['composition'] })
        );
        element.setAttribute(
          'integration-settings',
          JSON.stringify({ reactivity: 'manual', binding: 'one-way', debounce: 300 })
        );
    ...
    expect(JSON.parse(frameworkTest.updatedConfig || '{}')).toEqual({
      type: 'vue',
      version: '3.3',
      features: ['composition'],
    });
    expect(JSON.parse(frameworkTest.updatedSettings || '{}')).toEqual({
      reactivity: 'manual',
      binding: 'one-way',
      debounce: 300,
    });
  ...
  test('processes conditional attribute updates', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      element.setAttribute(
        'feature-flags',
        JSON.stringify({ advancedMode: false, betaFeatures: true })
      );
      element.setAttribute(
        'conditional-behavior',
        JSON.stringify({ showButtons: 'if-enabled', validation: 'if-required' })
      );
    ...
    await page.evaluate(() => {
      ...
      if (element) {
        element.setAttribute(
          'feature-flags',
          JSON.stringify({ advancedMode: true, betaFeatures: false, experimentalUI: true })
        );
        element.setAttribute(
          'conditional-behavior',
          JSON.stringify({ showButtons: 'always', validation: 'strict' })
        );
    ...
    expect(JSON.parse(conditionalTest.updatedFlags || '{}')).toEqual({
      advancedMode: true,
      betaFeatures: false,
      experimentalUI: true,
    });
    expect(JSON.parse(conditionalTest.updatedBehavior || '{}')).toEqual({
      showButtons: 'always',
      validation: 'strict',
    });
  ...
  test('updates component styling attributes', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      element.setAttribute(
        'css-variables',
        JSON.stringify({ '--primary-color': '#007bff', '--border-radius': '4px' })
      );
    ...
    expect(JSON.parse(stylingTest.updatedVariables || '{}')).toEqual({
      '--primary-color': '#6c757d',
      '--border-radius': '8px',
      '--font-size': '14px',
    });
