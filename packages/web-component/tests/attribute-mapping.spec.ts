...
test.describe('TouchSpin Web Component attribute to property mapping', () => {
  ...
  test('handles invalid attribute values gracefully', async ({ page }) => {
    ...
    const gracefulTest = await page.evaluate(() => {
      const element = document.querySelector(
        '[data-testid="graceful-handling-test"]'
      ) as HTMLElement;
  ...
  test('maps custom renderer attributes', async ({ page }) => {
    ...
    const rendererTest = await page.evaluate(() => {
      const element = document.querySelector(
        '[data-testid="renderer-mapping-test"]'
      ) as HTMLElement;
      ...
      const processRendererAttributes = () => {
        ...
        if (renderer === 'bootstrap5') {
          ...
          rendererConfig.classes = {
            ...
            buttonUp:
              element?.getAttribute('button-up-class') ||
              `btn btn-${rendererConfig.outline ? 'outline-' : ''}${rendererConfig.theme}`,
            buttonDown:
              element?.getAttribute('button-down-class') ||
              `btn btn-${rendererConfig.outline ? 'outline-' : ''}${rendererConfig.theme}`,
          };
        }
  ...
  test('handles complex attribute values', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      element.setAttribute(
        'custom-config',
        '{ "theme": "dark", "animation": { "duration": 300, "easing": "ease-out" } }'
      );
      element.setAttribute(
        'validation-rules',
        '{ "required": true, "pattern": "^[0-9]+$", "custom": function(val) { return val > 0; } }'
      );
      element.setAttribute('transform-function', 'Math.round');
      element.setAttribute(
        'format-options',
        '{ "locale": "en-US", "currency": "USD", "minimumFractionDigits": 2 }'
      );
      element.setAttribute('min', '0');
      element.setAttribute('max', '100');
      document.body.appendChild(element);
    ...
    const complexTest = await page.evaluate(() => {
      ...
      const parseComplexAttribute = (
        attrName: string,
        type: 'function' | 'json' | 'expression'
      ) => {
    ...
    expect(complexTest.complexAttributes['callback-before']).toBe(
      'function(val) { return val * 2; }'
    );
    expect(complexTest.complexAttributes['callback-after']).toBe(
      'function(val) { return val + " items"; }'
    );
  ...
  test('processes JSON attribute values', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      element.setAttribute(
        'theme-config',
        JSON.stringify({ primary: '#007bff', secondary: '#6c757d' })
      );
  ...
  test('supports dynamic attribute discovery', async ({ page }) => {
    ...
    const dynamicTest = await page.evaluate(() => {
      const element = document.querySelector(
        '[data-testid="dynamic-discovery-test"]'
      ) as HTMLElement;
  ...
  test('processes custom class attributes', async ({ page }) => {
    ...
    expect(customClassTest.customClassAttributes['wrapper-class']).toBe(
      'custom-wrapper my-wrapper'
    );
    expect(customClassTest.customClassAttributes['input-class']).toBe(
      'custom-input form-control-lg'
    );
    expect(customClassTest.customClassAttributes['button-class']).toBe('custom-button');
    expect(customClassTest.customClassAttributes['button-up-class']).toBe(
      'btn-custom-up text-success'
    );
    expect(customClassTest.customClassAttributes['button-down-class']).toBe(
      'btn-custom-down text-danger'
    );
    expect(customClassTest.customClassAttributes['prefix-class']).toBe('custom-prefix bg-light');
    expect(customClassTest.customClassAttributes['postfix-class']).toBe(
      'custom-postfix bg-secondary'
    );
    expect(customClassTest.customClassAttributes['group-class']).toBe('custom-group d-flex');
    expect(customClassTest.customClassAttributes['container-class']).toBe(
    ...
    expect(customClassTest.processedClasses.container).toEqual([
      'custom-container',
      'position-relative',
    ]);
  ...
  test('supports plugin-specific attributes', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      element.setAttribute(
        'animation-config',
        '{ "duration": 300, "easing": "ease-in-out", "delay": 50 }'
      );
      element.setAttribute(
        'accessibility-config',
        '{ "announceChanges": true, "keyboardNavigation": true }'
      );
    ...
    const pluginTest = await page.evaluate(() => {
      const element = document.querySelector(
        '[data-testid="plugin-attributes-test"]'
      ) as HTMLElement;
    ...
    expect(pluginTest.pluginAttributes['plugin-order']).toBe(
      'validation,format,animation,accessibility'
    );
    ...
    expect(pluginTest.processedPlugins.execution.order).toEqual([
      'validation',
      'format',
      'animation',
      'accessibility',
  ...
  test('handles conditional attribute processing', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      element.setAttribute(
        'condition-context',
        '{ "value": 50, "decimals": 2, "required": true, "currency": "USD", "mobile": false, "debug": true, "step": 5 }'
      );
      element.setAttribute('min', '0');
      element.setAttribute('max', '100');
      element.setAttribute('value', '50');
    ...
    const conditionalTest = await page.evaluate(() => {
      ...
      const processConditionalAttributes = () => {
        ...
        const evaluations = {
          ...
          disableUnless: conditions.disableUnless
            ? evaluateCondition(conditions.disableUnless, context)
            : false,
          showPrefixIf: conditions.showPrefixIf
            ? evaluateCondition(conditions.showPrefixIf, context)
            : false,
          hideButtonsUnless: conditions.hideButtonsUnless
            ? evaluateCondition(conditions.hideButtonsUnless, context)
            : false,
          validateIf: conditions.validateIf
            ? evaluateCondition(conditions.validateIf, context)
            : false,
          formatWhen: conditions.formatWhen
            ? evaluateCondition(conditions.formatWhen, context)
            : false,
          animateUnless: conditions.animateUnless
            ? !evaluateCondition(conditions.animateUnless, context)
            : true,
  ...
  test('supports attribute validation schemas', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      element.setAttribute(
        'validation-schema',
        '{ "type": "object", "properties": { "min": { "type": "number", "minimum": 0 }, "max": { "type": "number", "maximum": 1000 }, "step": { "type": "number", "exclusiveMinimum": 0 } } }'
      );
    ...
    const schemaTest = await page.evaluate(() => {
      const element = document.querySelector(
        '[data-testid="schema-validation-test"]'
      ) as HTMLElement;
      ...
      const processSchemaValidation = () => {
        ...
        const validateAttribute = (attrName: string, attrValue: string, schema: any) => {
          if (!schema?.properties?.[attrName]) {
            return strict
              ? { valid: false, error: 'Property not defined in schema' }
              : { valid: true };
          }
          ...
          if (propSchema.type === 'number') {
            ...
            if (propSchema.minimum !== undefined && numValue < propSchema.minimum) {
              return {
                valid: false,
                error: `Value ${numValue} below minimum ${propSchema.minimum}`,
              };
            }
            if (propSchema.maximum !== undefined && numValue > propSchema.maximum) {
              return {
                valid: false,
                error: `Value ${numValue} above maximum ${propSchema.maximum}`,
              };
            }
            if (
              propSchema.exclusiveMinimum !== undefined &&
              numValue <= propSchema.exclusiveMinimum
            ) {
              return {
                valid: false,
                error: `Value ${numValue} not greater than ${propSchema.exclusiveMinimum}`,
              };
            }
          }
  ...
  test('processes nested attribute structures', async ({ page }) => {
    ...
    const nestedTest = await page.evaluate(() => {
      const element = document.querySelector(
        '[data-testid="nested-attributes-test"]'
      ) as HTMLElement;
  ...
  test('handles attribute conflicts resolution', async ({ page }) => {
    ...
    const conflictTest = await page.evaluate(() => {
      const element = document.querySelector(
        '[data-testid="conflicts-resolution-test"]'
      ) as HTMLElement;
      const input = element?.querySelector('input') as HTMLInputElement;
  ...
  test('supports custom attribute extensions', async ({ page }) => {
    ...
    const extensionTest = await page.evaluate(() => {
      const element = document.querySelector(
        '[data-testid="custom-extensions-test"]'
      ) as HTMLElement;
      ...
      const processCustomExtensions = () => {
        ...
        registry.forEach((prefix) => {
          const prefixAttrs = attributes.filter(
            (attr) =>
    ...
    expect(extensionTest.extensionAttributes['x-config']).toBe(
      '{ "customFeature": true, "advancedMode": "pro" }'
    );
    ...
    expect(extensionTest.processedExtensions.customAttributes['x-custom-behavior'].value).toBe(
      'special'
    );
    expect(extensionTest.processedExtensions.customAttributes['x-custom-behavior'].prefix).toBe(
      'x-'
    );
    expect(extensionTest.processedExtensions.customAttributes['x-custom-behavior'].handler).toBe(
      'CustomExtensionProcessor'
    );
    expect(extensionTest.processedExtensions.customAttributes['x-custom-behavior'].processed).toBe(
      true
    );

    expect(extensionTest.processedExtensions.customAttributes['ext-validation'].value).toBe(
      'custom'
    );
    expect(extensionTest.processedExtensions.customAttributes['ext-validation'].prefix).toBe(
      'ext-'
    );

    expect(extensionTest.processedExtensions.customAttributes['custom-prefix'].value).toBe(
      'special-'
    );
    expect(extensionTest.processedExtensions.customAttributes['custom-prefix'].prefix).toBe(
      'custom-'
    );
  ...
  test('maps framework-specific attributes', async ({ page }) => {
    ...
    const frameworkTest = await page.evaluate(() => {
      const element = document.querySelector(
        '[data-testid="framework-attributes-test"]'
      ) as HTMLElement;
      ...
      const processFrameworkAttributes = () => {
        const frameworks = {
          angular: {
            ...
            detected: !!(
              element?.getAttribute('angular-directive') ||
              element?.getAttribute('angular-validator')
            ),
          },
          react: {
          ...
          svelte: {
            ...
            detected: !!(
              element?.getAttribute('svelte-bind') || element?.getAttribute('svelte-action')
            ),
          },
        };
    ...
    expect(frameworkTest.processedFrameworks.detectedFrameworks).toEqual([
      'angular',
      'react',
      'vue',
      'svelte',
    ...
    expect(frameworkTest.processedFrameworks.frameworkSupport).toBe(
