...
export async function initializeTouchspin(
  ...
  if (serializedOptions.callback_before_calculation && typeof serializedOptions.callback_before_calculation === 'function') {
    serializedOptions.callback_before_calculation = serializedOptions.callback_before_calculation.toString();
  }
  if (serializedOptions.callback_after_calculation && typeof serializedOptions.callback_after_calculation === 'function') {
    serializedOptions.callback_after_calculation = serializedOptions.callback_after_calculation.toString();
  }
  ...
  await page.evaluate(({ testId, options }) => {
    ...
    if (options.callback_before_calculation && typeof options.callback_before_calculation === 'string') {
      options.callback_before_calculation = new Function('return ' + options.callback_before_calculation)();
    }
    if (options.callback_after_calculation && typeof options.callback_after_calculation === 'string') {
      options.callback_after_calculation = new Function('return ' + options.callback_after_calculation)();
    }
    ...
    const core = {
      ...
      _checkCallbackPairing: () => {
        ...
        const hasBefore = settings.callback_before_calculation &&
          settings.callback_before_calculation.toString() !== defCb.toString();
        const hasAfter = settings.callback_after_calculation &&
...
export async function updateSettingsViaAPI(
  ...
  if (serializedSettings.callback_before_calculation && typeof serializedSettings.callback_before_calculation === 'function') {
    serializedSettings.callback_before_calculation = serializedSettings.callback_before_calculation.toString();
  }
  if (serializedSettings.callback_after_calculation && typeof serializedSettings.callback_after_calculation === 'function') {
    serializedSettings.callback_after_calculation = serializedSettings.callback_after_calculation.toString();
  }
  ...
  await page.evaluate(({ testId, newSettings }) => {
    ...
    // Reconstruct callback functions from strings
    if (newSettings.callback_before_calculation && typeof newSettings.callback_before_calculation === 'string') {
      newSettings.callback_before_calculation = new Function('return ' + newSettings.callback_before_calculation)();
    }
    if (newSettings.callback_after_calculation && typeof newSettings.callback_after_calculation === 'string') {
      newSettings.callback_after_calculation = new Function('return ' + newSettings.callback_after_calculation)();
    }
