...
test.describe('Core TouchSpin Events', () => {
  ...
  test.afterEach(async ({ page }) => {
    await apiHelpers.collectCoverage(page, 'core-events');
  });
  ...
  test.describe('Event Count Verification', () => {
    test('should emit exactly one startspin and one stopspin per continuous spin', async ({ page }) => {
      ...
      const startCount = await apiHelpers.countEventInLog(page, 'touchspin.on.startspin', 'touchspin');
      const stopCount = await apiHelpers.countEventInLog(page, 'touchspin.on.stopspin', 'touchspin');
    ...
    test('should emit exactly one change event per value change', async ({ page }) => {
      ...
      const changeCount = await apiHelpers.countEventInLog(page, 'change', 'native');
    ...
    test('should emit min event every time minimum boundary is hit', async ({ page }) => {
      ...
      const minCount = await apiHelpers.countEventInLog(page, 'touchspin.on.min', 'touchspin');
    ...
    test('should emit max event every time maximum boundary is hit', async ({ page }) => {
      ...
      const maxCount = await apiHelpers.countEventInLog(page, 'touchspin.on.max', 'touchspin');
