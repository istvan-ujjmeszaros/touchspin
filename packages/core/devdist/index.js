...
export class TouchSpinCore {
    ...
    constructor(inputEl, opts = {}) {
        ...
        this._settingObservers = new Map();
    ...
    _sanitizeSettings() {
        ...
        if (!isFinite(si) || si < 0)
            this.settings.stepinterval = DEFAULTS.stepinterval;
        // stepintervaldelay
    ...
    _validateCallbacks() {
        ...
        if (!/^-?\d*\.?\d*$/.test(afterResult)) {
            console.warn('TouchSpin: Detected formatting callback that adds non-numeric characters. ' +
    ...
    _captureOriginalAttributes() {
        if (this._originalAttributes !== null)
    ...
    _syncSettingsFromNativeAttributes() {
        ...
        if (nativeStep !== null) {
            ...
            const stepNum = parsedStep !== undefined && isFinite(parsedStep) && parsedStep > 0 ? parsedStep : undefined;
    ...
    _checkCallbackPairing() {
        ...
        if (hasBefore && !hasAfter) {
            console.warn('TouchSpin: callback_before_calculation is defined but callback_after_calculation is missing. ' +
                'These callbacks should be used together - one removes formatting, the other adds it back.');
        }
        else if (!hasBefore && hasAfter) {
            console.warn('TouchSpin: callback_after_calculation is defined but callback_before_calculation is missing. ' +
                'These callbacks should be used together - one removes formatting, the other adds it back.');
        }
    }
}
...
export function getTouchSpin(inputEl) {
    return inputEl[INSTANCE_KEY]
        ? inputEl[INSTANCE_KEY].toPublicApi()
        : null;
}
/**
