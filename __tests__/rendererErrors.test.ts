...
test.describe('Renderer Error Handling and Edge Cases', () => {
  ...
  test.describe('Renderer State Management Edge Cases', () => {
    test('should handle prefix/postfix updates without renderer', async ({ page }) => {
      const errorMessage = await page.evaluate(() => {
        try {
          const $ = (window as any).jQuery;
          $('body').append('<input id="prefix-fail-test" type="text" value="50" data-testid="prefix-fail-test">');
          const $input = $('#prefix-fail-test');
          
          // Initialize TouchSpin normally
          $input.TouchSpin();
          
          // Now corrupt the renderer reference
          const touchspin = $input.data('touchspin');
          if (touchspin) {
            touchspin.renderer = null;
            
            // Try to update settings that require renderer
            $input.trigger('touchspin.updatesettings', { prefix: 'new-prefix' });
          }
    ...
    test('should handle element access without initialized renderer', async ({ page }) => {
      const errorMessage = await page.evaluate(() => {
        try {
          const $ = (window as any).jQuery;
          $('body').append('<input id="element-access-fail-test" type="text" value="50" data-testid="element-access-fail-test">');
          const $input = $('#element-access-fail-test');
          ...
          if (touchspin) {
            touchspin.renderer = null;
            touchspin.elements = null;
            
            // Try to trigger actions that need elements
